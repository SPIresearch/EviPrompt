...
class SamPredictor:
    def __init__(
        ...
        def farward_hook(module, inp, outp):
            self.features = outp.squeeze() 
        self.model.mask_decoder.output_upscaling.register_forward_hook(farward_hook)
    ...
    def set_image(
        ...
        input_image_torch = image.unsqueeze(0)
    ...
    def predict(
        ...
        return masks[0].detach().cpu().numpy(), iou_predictions[0].detach().cpu().numpy(), low_res_masks[0].detach().cpu().numpy(), high_res_masks[0]
