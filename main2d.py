...
from vis_tools import point_selection, show_points, show_mask, patch_select_point, patch_select_point_rl
...
def get_arguments():
    ...
    parser.add_argument('--dataset', type=str, default='JSRT')
    
    parser.add_argument('--img_size', type=int, default=512)
    parser.add_argument('--num_grid_point', type=int, default=8)
    parser.add_argument('--patch_size', type=int, default=4)
    parser.add_argument('--num_anchors_pos', type=int, default=10)
...
def main():
    ...
    print('======> Start Testing')
    lines = open(args.dataset+'.txt').readlines()

    data_list = []
    for l in lines:
        reci = l.strip().split(' ')
        data_list.append(reci)

    ref_folder = os.path.join(args.root, 'Segmentation01_Index','ref')
    ref_img =  read_img(os.path.join(ref_folder,  f'img2.png'))

    ref_gt =  read_lbl(os.path.join(ref_folder,  f'mask2.png'),True)
    ...
    for img, gt in tqdm(data_list):
      
        tgt_img = read_img(img)#os.path.join(test_img_folder, img))
        tgt_gt = read_lbl(gt)#(os.path.join(test_gt_folder, img))
        ...
        # topk_xy_pos, topk_label_pos, _, _ = point_selection(b[:,:,1], args.num_point_pos)
        # topk_xy_neg, topk_label_neg, _, _ = point_selection(b[:,:,0], args.num_point_neg)
        ...
        # for jsrt
        topk_xy_pos, _ = patch_select_point_rl(b[:,:,1], args.patch_size, args.num_point_pos)
        topk_xy_neg, _ = patch_select_point_rl(b[:,:,0], args.patch_size, args.num_point_neg)
        ...
        point_lbl = np.concatenate([np.ones(args.num_point_pos*2, dtype=int), np.zeros(args.num_point_neg*2, dtype=int)],0)
    ...
    dataset_size = len(DSCs)
    DSCs, NSDs = np.array(DSCs).mean(), np.array(NSDs).mean()
    print(f'final results: size: {dataset_size}, DSC: {DSCs*100.:.2f}, NSD: {NSDs*100.:.2f}')
